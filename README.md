...
> **Beta 开发中，不能作为真实升级决策的唯一依据。** 当前 catalog 有 owner 审批的两条 exact support edge 和六条规则，仅限 [support matrix](catalog/support-matrix.yaml) 明确列出的范围。P07 的安全与性能验收及 P08 的有限实验 beta 验收已经完成；P08 仅覆盖 `1.16.0 → 1.16.1` 的核心服务画像，不覆盖 worker edge、Nginx、SSRF proxy、恢复或生产环境。P09 仍在本地发布候选准备中，未发布、未收集真实维护者反馈；`NO_KNOWN_BLOCKERS` 不是升级安全保证。

## 当前可运行内容

需要 Python 3.11+。在新的项目虚拟环境中安装后，可运行合成闭环、只读 snapshot 和离线 check：
...
`demo` 的 `BLOCKED` 退出码为 2；这是固定协议示例，不表示任何真实 Dify 部署存在问题。产品不连接数据库、Docker daemon 或生产目录；snapshot 仅接受调用者明确指定的输入，并遵守 [SAFETY_CONTRACT.md](SAFETY_CONTRACT.md)。完整命令、退出码与限制见 [用户指南](docs/USER_GUIDE.md)。

## 项目边界与状态

- [PROJECT_CHARTER.md](PROJECT_CHARTER.md) 说明产品范围；[ACCEPTANCE_CONTRACT.md](ACCEPTANCE_CONTRACT.md) 定义完成门槛。
- [roadmap.yaml](roadmap.yaml) 是唯一阶段状态来源。P09.T1 正在整理本地候选；不得将 matrix 之外的版本或 `NO_KNOWN_BLOCKERS` 解读为安全保证。
