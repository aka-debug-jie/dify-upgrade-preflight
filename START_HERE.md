...
**本仓库同时包含开发契约、设计、测试规格和早期实现。当前没有任何已认证升级路径、生产规则或性能实测结果。**
...
3. 先读取 `roadmap.yaml`。当前 P00–P04 已完成；P05 停在 `blocked_review`，需先完成候选证据与案例的人工裁决，不能直接开始后续阶段。
...
- 已批准真实支持矩阵为空。这是诚实的初始状态，不是让模型自行填充的空白。
